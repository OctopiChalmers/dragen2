...
%
With this representation, the page \texttt{<html>hello<hr>bye</html>} can be
encoded as:
%
\begin{code}
Tag "html" (Join (Join
...
the \emph{generation size}---capture in the code with variable |size|.
%
...
among any |Html| data constructor of with an explicitly given \emph{generation
  frequency} that can be chosen by the user---in this example, 2, 1, 4 and 3 for
|Text|, |Single|, |Tag|, and |Join|, respectively.
%
...
\footnote{%
  ...
  from calling random generators and they are not particularly relevant for the
  point being made in this work.}
%$
...
%
\dragen \cite{DBLP:conf/haskell/MistaRH18} is a tool conceived to mitigate the
problem of finding the appropriated generation frequencies.
%
It uses the theory of branching processes \cite{gw1875} to modelize and predict
analytically the expected number of generated data constructors.
%
...
As many other tools for automatic derivation of generators
...
generation process is limited to pick \emph{a single data constructor at the
  time and then recursively generate each required subexpression independently}.
%
