...
a single transformation \ensuremath{\Varid{step}} from an \ensuremath{\text{HRep}} back into a concrete value of the
target:
%
\begin{align*}
...
for the canonical \ensuremath{\text{HRep}} of data constructors simply by mapping each lifted
...
\begin{alignat*}{4}
  ...
  &\evalrep{\ensuremath{\text{Mk}_{Add}}\ x\ y   &&}{\ensuremath{\Conid{Exp}}}
    &&= \ensuremath{\Conid{Add}}\ \evalrep{x}{\ensuremath{\Conid{Exp}}}\ \evalrep{y}{\ensuremath{\Conid{Exp}}} \\
  &\evalrep{\ensuremath{\text{Mk}_{Mul}}\ x\ y   &&}{\ensuremath{\Conid{Exp}}}
    &&= \ensuremath{\Conid{Mul}}\ \evalrep{x}{\ensuremath{\Conid{Exp}}}\ \evalrep{y}{\ensuremath{\Conid{Exp}}}
\end{alignat*}
...
In order to compose the \ensuremath{\text{HRep}}s for the single data constructors already
presented, we will need to define a series of type level combinators.

In first place, we define a type combinator ($\otimes$) to tag a \ensuremath{\text{HRep}} with an
...
\column{B}{@{}>{\hspre}l<{\hspost}@{}}%
\column{E}{@{}>{\hspre}l<{\hspost}@{}}%
\>[B]{}gen_{f \otimes n}\;gen_{r}\mathrel{=}\Conid{Term}\mathop{\mathsmaller{\langle \$ \rangle}}gen_{f}\;gen_{r}{}\<[E]%
\ColumnHook
\end{hscode}\resethooks
...
exhausted.
%
For this, we follow the same approach as before to define a combinator (\ensuremath{\_^\bullet})
...
\column{B}{@{}>{\hspre}l<{\hspost}@{}}%
\column{E}{@{}>{\hspre}l<{\hspost}@{}}%
\>[B]{}gen_{f^\bullet}\;gen_{r}\mathrel{=}\Conid{Term}\mathop{\mathsmaller{\langle \$ \rangle}}gen_{f}\;gen_{r}{}\<[E]%
\ColumnHook
\end{hscode}\resethooks


With these individual representations, we can define a type combinator
$(\oplus)$ to compose two \ensuremath{\text{HRep}}s into a single one:
...
tranformation to the inner \ensuremath{\text{HRep}}:
%
\begin{alignat*}{3}
...
to refer to the canonical \ensuremath{\text{HRep}} of our original data type \ensuremath{\Conid{Exp}}, tagging for
instance the representation of the constuctor \ensuremath{\Conid{Add}} to be generated in with
