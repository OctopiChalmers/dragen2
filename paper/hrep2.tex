...
In this section, we present how to provide the structural information express in
ADTs definitions, patterns matchings, and abstract interfaces to our automatic
process of deriving random data generators.
%
...
%
Based on the codebase, the user of our tool (\dragenp) indicates: (i) the ADT
definition to consider (noted as $Html_{ADT}$), (ii) its patterns of interest
(noted $Html_{Patterns}$), and (iii) primitives from abstract interfaces worth
to involve in the generation process (noted as $Html_{Interface}$).
%
...
occurrences of |Html|, e.g., it generates values of the form (|Text "xA2sx"|),
(|Single "xj32da"|), (|Tag "divx234jx" hole|) and (|Join hole hole|), where
|hole| is a placeholder denoting an incomplete value.
%
%
Similarly, the generator obtained from $Html_{Patterns}$ generates values
satisfying the expected patterns where recursive calls are once more left
...
primitives in the indicated interface, where each argument of type |Html| is
left uncompleted, e.g., (|div hole|) and (|bold hole|).
%
...
|"bdfxwe"|, ADT constructor (|Text "xx34s"|), pattern (|Join (Join (Text hole)
hole)|, and interface call (|div hole|), we can obtain the following well-formed
|Html| test case:
%
\begin{code}
...
of constructors, patterns, and interface calls that the generator will
produce (explained in next Section).
%
Overall, our approach offers significant advantages over usual
derivation of random generators based only on ADT definitions:
%
...
\item \textbf{Extensibility:} the developer can specify new sources of
  ...
  adding them to the existing specification of the target data type.
  %
\item \textbf{Predictability:} the tool is capable to synthesize generators
  ...
  uniform expected number of generated patterns, while some constructors should
  be generated twice as much as others, etc. We will explain that in the next
  Section.
  %
