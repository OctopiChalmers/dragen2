...
Random testing is a promising approach to finding\todoale{find?} bugs
...
random generators.
%
More specifically, we propose a generation process that is capable to consider
how programs branch on input ADTs values as well as how they get manipulated by
abstract interfaces (Section \ref{sec:hrepcont}).
%
...
We implement our ideas in a tool called \dragenp\footnote{\dragenp is available
  at \url{http://github.com/OctopiChalmers/dragen-struct}}, that is capable to
automatically synthesize \quickcheck generators capable to produce \emph{rich ADT values},
where the distributions of random values can be adjusted at compile-time to what
